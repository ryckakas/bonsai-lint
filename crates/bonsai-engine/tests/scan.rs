...
#[test]
fn a_go_file_is_discovered_and_scored_as_go() {
    let (_dir, root) = project();
    write(&root, "pkg/f.go", GO_UNIT.as_bytes());
    write(&root, "src/plain.ts", TS_UNIT.as_bytes());

    let only_go = scan(&root, &["."], Some(&["go".to_string()]));

    assert_eq!(only_go.stats.files, 1);
    let languages: Vec<_> = only_go
        .located
        .iter()
        .map(|located| located.finding.language)
        .collect();
...
/// Go marks machine-written files in their contents, not their names, so the file has to be read
/// before it can be turned away. It must still count as unscanned, the way a `.min.js` does.
#[test]
fn a_generated_go_file_is_neither_scored_nor_counted() {
    let (_dir, root) = project();
    let generated = format!("// Code generated by stringer. DO NOT EDIT.\n\n{GO_UNIT}");
    write(&root, "pkg/kind_string.go", generated.as_bytes());
...
/// A generated file is never scored, so a lossy decode of it has nothing worth a warning.
#[test]
fn a_generated_go_file_with_invalid_utf8_prints_no_warning() {
    let (_dir, root) = project();
    write(
        &root,
        "pkg/kind_string.go",
        b"// Code generated by stringer. DO NOT EDIT.\n\npackage p\n\n// caf\xE9\n",
    );
...
#[test]
fn a_marker_below_the_package_clause_does_not_make_a_file_generated() {
    let (_dir, root) = project();
    let source =
        "package p\n\n// Code generated by hand. DO NOT EDIT.\nfunc f() int { return 1 }\n";
    write(&root, "pkg/f.go", source.as_bytes());
...
#[test]
fn a_java_file_is_discovered_and_scored_as_java() {
    let (_dir, root) = project();
    write(&root, "src/main/java/p/F.java", JAVA_UNIT.as_bytes());
    write(&root, "src/plain.ts", TS_UNIT.as_bytes());

    let only_java = scan(&root, &["."], Some(&["java".to_string()]));

    assert_eq!(only_java.stats.files, 1);
    let findings: Vec<_> = only_java
        .located
        .iter()
        .map(|located| (located.finding.language, located.finding.qualified_name()))
        .collect();
    assert_eq!(findings, vec![("java", "F::f()".to_string())]);
}

/// Java marks generated sources in their header comment, so, as with Go, the file is read and
/// then turned away uncounted.
#[test]
fn a_generated_java_file_is_neither_scored_nor_counted() {
    let (_dir, root) = project();
    let generated = format!(
        "// Generated by the protocol buffer compiler.  DO NOT EDIT!
{JAVA_UNIT}"
    );
    write(&root, "src/p/Orders.java", generated.as_bytes());
...
#[test]
fn a_python_file_is_discovered_and_scored_as_python() {
    let (_dir, root) = project();
    write(&root, "app/f.py", PYTHON_UNIT.as_bytes());
    write(&root, "app/f.pyi", PYTHON_UNIT.as_bytes());
    write(&root, "src/plain.ts", TS_UNIT.as_bytes());

    let only_python = scan(&root, &["."], Some(&["python".to_string()]));

    assert_eq!(only_python.stats.files, 1);
    let findings: Vec<_> = only_python
        .located
        .iter()
        .map(|located| (located.finding.language, located.finding.qualified_name()))
        .collect();
    assert_eq!(findings, vec![("python", "F::f".to_string())]);
}

#[test]
fn a_generated_python_file_is_neither_scored_nor_counted() {
    let (_dir, root) = project();
    let generated = format!(
        "# -*- coding: utf-8 -*-
# Generated by the protocol buffer compiler.  DO NOT EDIT!
{PYTHON_UNIT}"
    ...
    write(&root, "app/orders_pb2.py", generated.as_bytes());
